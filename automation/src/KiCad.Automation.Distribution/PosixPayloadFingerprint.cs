...
internal static class PosixPayloadFingerprint
{
    internal static async Task<string> ComputeAsync(string root, CancellationToken token)
    {
        if (!OperatingSystem.IsLinux() && !OperatingSystem.IsMacOS()) throw new PlatformNotSupportedException("Unix payload modes require Linux or macOS.");
    ...
    {
        if (OperatingSystem.IsWindows()) throw new PlatformNotSupportedException("Unix file modes are unavailable on Windows.");
