...
internal static class PosixUpdateActivation
{
        ...
        string nextVersion, Guid operationId, Action? beforeSwitch, CancellationToken token)
    {
        ...
        if ((OperatingSystem.IsMacOS() ? RenameMac(pending, link) : Rename(pending, link)) != 0)
    ...
    {
        if (!OperatingSystem.IsLinux() && !OperatingSystem.IsMacOS()) throw new PlatformNotSupportedException("This selection store requires Linux or macOS.");
    }
    ...
    [DllImport("/usr/lib/libSystem.B.dylib", EntryPoint = "rename", SetLastError = true)]
    private static extern int RenameMac([MarshalAs(UnmanagedType.LPUTF8Str)] string oldPath,
