...
public sealed record LinuxRestartConfiguration(int SchemaVersion, LinuxUpdateHandoffRequest Request);

public static class LinuxRestartCommand
{
    ...
    public static async Task<int> RunAsync(string[] args, TextWriter output, CancellationToken token)
    {
        ...
        try
        {
            ...
            var configuration = JsonSerializer.Deserialize<LinuxRestartConfiguration>(bytes, Json);
            if (configuration is null || configuration.SchemaVersion != 1 || configuration.Request is null)
                throw new InvalidDataException("A versioned restart request is required.");
            var result = await LinuxUpdateHandoff.ExecuteAsync(configuration.Request, async state =>
            {
