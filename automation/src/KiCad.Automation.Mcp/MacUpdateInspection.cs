...
public sealed record MacUpdateInspectionResult(string Status, Guid OperationId, DateTimeOffset ObservedAtUtc,
...
public static class MacUpdateInspection
{
    private static readonly JsonSerializerOptions Json = new(JsonSerializerDefaults.Web)
    { PropertyNameCaseInsensitive = false, UnmappedMemberHandling = JsonUnmappedMemberHandling.Disallow, MaxDepth = 16 };

    public static async Task<MacUpdateInspectionResult> InspectAsync(string installationRoot, Guid operationId,
        CancellationToken token = default) =>
        (await InspectCoreAsync(installationRoot, operationId, token)) with { ObservedAtUtc = DateTimeOffset.UtcNow };

    private static async Task<MacUpdateInspectionResult> InspectCoreAsync(string installationRoot, Guid operationId,
        CancellationToken token = default)
    {
        if (!OperatingSystem.IsMacOS()) throw new PlatformNotSupportedException("Update inspection requires macOS.");
        ...
        var report = new MacUpdateInspectionResult("journal_missing", operationId, DateTimeOffset.UtcNow);
        ...
        using (ownership)
        {
            ...
            string oldExecutable = intent.PreviousVersion.NativeExecutable;
            ...
            string executable = restored ? oldExecutable
                : Path.Combine(root, "versions", request.ManifestSha256, "payload/install/KiCad.app/Contents/MacOS/kicad");
            _ = await MacVerifiedInstallation.InspectExecutableVersionAsync(root, executable, token);
    ...
    internal static async Task<MacUpdateJournalSnapshot> ReadSnapshotAsync(string root, string journal, Guid operation,
        CancellationToken token)
    {
        var intent = await ReadAsync<MacUpdateHandoffIntent>(Path.Combine(journal, "intent.json"), token);
        var request = await ReadAsync<MacUpdateHandoffRequest>(Path.Combine(journal, "request.json"), token);
        var state = await ReadAsync<MacUpdateHandoffState>(Path.Combine(journal, "state.json"), token);
        MacUpdateRecoveryClaim? recovery = null;
        if (File.Exists(Path.Combine(journal, "recovery.json")))
        {
            recovery = await ReadAsync<MacUpdateRecoveryClaim>(Path.Combine(journal, "recovery.json"), token);
            ...
            if (await ReadAsync<MacUpdateRecoveryClaim>(prefix + ".json", token) != recovery)
                throw new InvalidDataException("The current recovery pointer does not match its immutable attempt.");
            state = await ReadAsync<MacUpdateHandoffState>(prefix + ".state.json", token);
        }
        Validate(root, journal, operation, intent, request, state, recovery);
        string executable = intent.PreviousVersion.NativeExecutable;
    ...
    internal static string ObserveProcess(MacProcessIdentity expected, string executable)
    {
        try
        {
            using var process = Process.GetProcessById(expected.ProcessId);
            if (process.HasExited) return "exited";
            if (MacProcessIdentity.Read(process.Id) != expected) return "identity_changed";
            if (expected.Executable != MacProcessIdentity.PhysicalExecutable(executable)) return "executable_mismatch";
            if (process.HasExited) return "exited";
            return MacProcessIdentity.Read(process.Id) == expected ? "live" : "identity_changed";
        }
        ...
        MacUpdateHandoffRequest request, MacUpdateHandoffState state, MacUpdateRecoveryClaim? recovery)
    {
        if (intent.SchemaVersion != 1 || intent.Request != request || intent.PreviousVersion is null
            ...
            || request.OldProcess.StartSeconds == 0 || request.OldProcess.StartMicroseconds >= 1000000
        ...
        if (state.ProcessId is null && (state.ProcessIdentity is not null || state.Endpoint is not null)
            || state.ProcessId is <= 0 || state.ProcessIdentity is not null
                && (state.ProcessIdentity.ProcessId != state.ProcessId || state.ProcessIdentity.BootId == Guid.Empty
                    || state.ProcessIdentity.StartSeconds == 0 || state.ProcessIdentity.StartMicroseconds >= 1000000
                    || !Path.IsPathFullyQualified(state.ProcessIdentity.Executable))
