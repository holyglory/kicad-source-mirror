...
[TestClass]
public sealed class PosixUpdateActivationTests
{
    [TestMethod]
    ...
    {
        if (!OperatingSystem.IsLinux() && !OperatingSystem.IsMacOS()) { Assert.Inconclusive("Unix link activation requires Linux or macOS."); return; }
        using var fixture = new ActivationFixture(prefix);
        await PosixUpdateActivation.InitializeAsync(fixture.Installation, fixture.First);
        string previous = PosixUpdateActivation.InspectTarget(fixture.Installation);
        await using var live = File.OpenRead(Path.Combine(fixture.Installation, "current", "version.txt"));
        Guid operation = Guid.NewGuid();
        var switched = await PosixUpdateActivation.SwitchAsync(fixture.Installation, previous, fixture.Second, operation);
        ...
        Assert.AreEqual(switched, await PosixUpdateActivation.SwitchAsync(fixture.Installation, previous, fixture.Second, operation));
        var rollback = await PosixUpdateActivation.SwitchAsync(fixture.Installation, switched.Target, fixture.First, Guid.NewGuid());
        ...
        await Assert.ThrowsExactlyAsync<InvalidDataException>(() =>
            PosixUpdateActivation.SwitchAsync(fixture.Installation, previous, fixture.Second, operation));
        Assert.AreEqual(rollback.Target, PosixUpdateActivation.InspectTarget(fixture.Installation));
    ...
    public async Task ConflictingPointerOrPendingLinkIsPreservedUntilExplicitRepair()
    {
        ...
        await Assert.ThrowsExactlyAsync<InvalidDataException>(() =>
            PosixUpdateActivation.SwitchAsync(fixture.Installation, previous, fixture.Second, Guid.NewGuid()));
        ...
        Assert.AreEqual(previous, PosixUpdateActivation.InspectTarget(fixture.Installation));
        File.Delete(pending);
        var recovered = await PosixUpdateActivation.SwitchAsync(fixture.Installation, previous, fixture.Second, operation);
        Assert.AreEqual(recovered.Target, PosixUpdateActivation.InspectTarget(fixture.Installation));
    ...
    public async Task StaleIdsChangedOperationsCancellationAndLockedStoreNeverSwitch()
    {
        ...
        string previous = PosixUpdateActivation.InspectTarget(fixture.Installation);
        await Assert.ThrowsExactlyAsync<InvalidDataException>(() =>
            PosixUpdateActivation.SwitchAsync(fixture.Installation, "stale", fixture.Second, Guid.NewGuid()));
        using (var cancelled = new CancellationTokenSource())
        {
            cancelled.Cancel();
            await Assert.ThrowsAsync<OperationCanceledException>(() =>
                PosixUpdateActivation.SwitchAsync(fixture.Installation, previous, fixture.Second, Guid.NewGuid(), cancelled.Token));
        }
        ...
        var switched = await PosixUpdateActivation.SwitchAsync(fixture.Installation, previous, fixture.Second, operation);
        await Assert.ThrowsExactlyAsync<InvalidDataException>(() =>
            PosixUpdateActivation.SwitchAsync(fixture.Installation, previous, fixture.First, operation));
        Assert.AreEqual(switched.Target, PosixUpdateActivation.InspectTarget(fixture.Installation));
        await Assert.ThrowsAsync<IOException>(() =>
            PosixUpdateActivation.InitializeAsync(fixture.Installation, fixture.First));
    }
