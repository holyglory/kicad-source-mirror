...
public static class MacDependencyAudit
{
    ...
    private const string Helpers = """
        ...
        function(audit_group group main output)
          ...
          # Nested editor Frameworks links can name the same physical library.
          # Only collapse aliases of one file, never distinct library copies.
          foreach(filename IN LISTS conflict_FILENAMES)
            set(actual_paths)
            foreach(path IN LISTS conflict_${filename})
              file(REAL_PATH "${path}" actual)
              list(APPEND actual_paths "${actual}")
            endforeach()
            list(REMOVE_DUPLICATES actual_paths)
            list(LENGTH actual_paths count)
            if(count EQUAL 1)
              list(APPEND resolved ${actual_paths})
              list(REMOVE_ITEM conflict_FILENAMES "${filename}")
            endif()
          endforeach()
          list(REMOVE_DUPLICATES resolved)
          if(unresolved OR conflict_FILENAMES)
            set(diagnostic "{\"schemaVersion\":1,\"resolved\":[],\"unresolved\":[],\"conflicts\":[]}")
            json_string("${group}" group_json)
            string(JSON diagnostic SET "${diagnostic}" group "${group_json}")
            foreach(kind IN ITEMS resolved unresolved)
              set(index 0)
              foreach(path IN LISTS ${kind})
                json_string("${path}" path_json)
                string(JSON diagnostic SET "${diagnostic}" ${kind} ${index} "${path_json}")
            ...
            foreach(filename IN LISTS conflict_FILENAMES)
              set(entry "{\"paths\":[]}")
              json_string("${filename}" name_json)
              string(JSON entry SET "${entry}" name "${name_json}")
              set(path_index 0)
              foreach(path IN LISTS conflict_${filename})
                json_string("${path}" path_json)
                string(JSON entry SET "${entry}" paths ${path_index} "${path_json}")
                math(EXPR path_index "${path_index}+1")
              endforeach()
              string(JSON diagnostic SET "${diagnostic}" conflicts ${index} "${entry}")
              math(EXPR index "${index}+1")
            endforeach()
            file(WRITE "${report_path}.failure.json" "${diagnostic}")
            list(LENGTH unresolved missing_count)
            list(LENGTH conflict_FILENAMES conflict_count)
            message(FATAL_ERROR "${group}: ${missing_count} unresolved, ${conflict_count} conflicts; see ${report_path}.failure.json")
          endif()
