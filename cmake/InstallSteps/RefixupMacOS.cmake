...
function( mac_image_needs_rpath loads identity output )
    # Enumerate identifiers rather than removing a first line with an anchored
    # global replacement (whose repeated-match behavior differs across CMake).
    string( REPLACE "\n" ";" identity_lines "${identity}" )
    list( TRANSFORM identity_lines STRIP )
    string( REPLACE "\n" ";" load_lines "${loads}" )
    ...
    foreach( line IN LISTS load_lines )
        ...
        if( dependency MATCHES "^@rpath/" AND NOT dependency IN_LIST identity_lines )
            set( needs_rpath TRUE )
        endif()
    endforeach()
    set( ${output} ${needs_rpath} PARENT_SCOPE )
endfunction()
...
function( refix_image_rpaths binary bundle )
    ...
    mac_image_needs_rpath( "${loads}" "${identity}" needs_rpath )
